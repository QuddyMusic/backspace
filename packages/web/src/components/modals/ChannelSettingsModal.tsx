...
function OverviewTab({
  ...
  // (the server normalizes what we sent, or someone else renames the channel)
  // — but never while the user is typing, that would clobber their edit.
  useEffect(() => {
    if (!isEditingName) setEditName(channelName);
  }, [channelName, isEditingName]);
  ...
  // click must not rename the channel) and Enter is not a submit either;
  ...
  const saveName = async () => {
    ...
    try {
      ...
      // normalization (trimmed, lowercased, whitespace → dashes).
...
export function ChannelSettingsModal() {
  ...
  const handleRenameChannel = async (name: string): Promise<string> => {
    if (!channelId || !currentSpaceId) return name;
    setError('');
    const origin = space?._instanceOrigin ?? '';
    const channelApi = getApiForOrigin(origin);
    try {
      const updated = await channelApi.channels.update(channelId, { name });
      upsertChannel(updated, currentSpaceId, origin);
